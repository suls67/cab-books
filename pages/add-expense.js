...
export default function AddExpense() {
  ...
  async function handleSubmit(event) {
    ...
    if (!amount) {
      setStatus({ type: 'error', message: 'Enter the expense amount before saving.' })
      return
    }
    ...
    if (error) {
      setStatus({ type: 'error', message: `Could not save expense: ${error.message}` })
      setIsSaving(false)
      return
    }
    ...
    <div className={styles.container}>
      <div className={styles.shell}>
        <div className={styles.header}>
          <div>
            <p className={styles.eyebrow}>Expense entry</p>
            <h1>Add expense</h1>
            <p className={styles.subtitle}>
              Save a cost, charge, or operating expense to your dashboard.
            </p>
          </div>
        ...
        <div className={styles.card}>
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            <div className={styles.actions}>
              <button
                type="submit"
                className={`${styles.primaryBtn} ${styles.expensePrimaryBtn}`}
                disabled={isSaving}
              >
